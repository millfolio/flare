...
# Per-connection state-machine constants, ``StepResult``, the
# h2c-upgrade detector, and the byte-fast-path / keep-alive
# helpers live in ``flare.http._reactor.conn_handle``. They are
# re-exported here for back-compat with every existing import
# site (``flare.http``, ``flare.http2``, ``flare.runtime``,
# ``tests/``, ``fuzz/``); a follow-up commit moves ``ConnHandle``
# itself into the same sub-package.
