"""Tests for ``flare.http.proto.h1_leniency.H1LeniencyConfig``.

The struct is the public API contract; parser plumbing for each
flag is a follow-up. These tests pin the public surface so the
contract can't drift before the plumbing lands.
"""
...
def test_copy_and_move() raises:
    var c = H1LeniencyConfig(allow_obs_fold=True)
    var copied = c.copy()
